...
final class NOVACertificateSyncService {
    ...
    func sync(
        ...
        do {
            ...
            guard
                let package =
                    try await auth.fetchCertificate()
            else {
                return
            }
            ...
            guard
                let profileData =
                    Data(
                        base64Encoded:
                            package.profileBase64
                    )
            else {
                print(
                    "NOVA: Invalid profile Base64"
                )
                return
            }
