...
private enum TabIconImage {
    static func make(inner: String, selected: Bool) -> UIImage {
        ...
        return renderer.image { context in
            ...
            guard let symbol = UIImage(systemName: inner) else {
                return
            }
...
final class ForgeApplicationDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(
        ...
        guard let url =
            socialURL(for: shortcutItem.type)
        else {
...
struct ForgeSignMobileApp: App {
    ...
    @StateObject private var certificates =
        CertificateStore()

    @StateObject private var profiles =
        ProfileStore()

    @StateObject private var history =
        HistoryStore()

    @StateObject private var installer =
        InstallController()

    @StateObject private var repositories =
        RepositoryStore()
...
private struct ForgeRootView: View {
    ...
    @EnvironmentObject
    private var installer: InstallController
