...
final class CertificateStore: ObservableObject {
    ...
    enum ImportError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .badPassword:
                return "Wrong password, or not a valid signing certificate."

            case .copyFailed:
                return "The certificate could not be saved."
            }
        }
    }
    ...
    private func importCertificateData(
        data: Data,
        ...
        // إذا الشهادة نفسها موجودة، نحذف النسخة القديمة
        // حتى لا تتكرر عند كل تسجيل دخول.
        if let existing =
            certificates.first(
                where: {
...
enum PasswordVault {
    ...
    private static func fallbackURL(
        ...
        fallbackDir.appendingPathComponent(
            id.uuidString
        )
    }
    ...
    static func save(
        ...
        try? Data(password.utf8).write(
            to: fallbackURL(for: id),
            options: .completeFileProtection
        )
    }
